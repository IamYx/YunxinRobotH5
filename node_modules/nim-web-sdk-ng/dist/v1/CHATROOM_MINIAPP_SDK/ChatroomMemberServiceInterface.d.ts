...
export interface ChatroomMemberServiceInterface {
     ...
     * @example
     * ```ts
     * const member = await chatroom.chatroomMember.setMemberNormal({
     ...
     * @example
     * ```ts
     * const member = await chatroom.chatroomMember.setMemberMute({
     *   account: 'target-account',
     *   isAdd: true
     * })
     * ```
...
export interface UpdateMyRoomRoleOptions {
     ...
     * @Multi_Lang_Tag
     * @locale cn
     * 可选，默认false，是否支持 nick, avatar 和 ext 字段的持久化，固定成员有效
     * @locale
     *
     * @locale en
